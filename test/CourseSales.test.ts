...
describe('Primary Course Sales and Backer Revenue Distribution', function () {
  async function deployCourseSalesFixture() {
    const [creator, backer1, backer2, buyer1, buyer2, platform] =
    ...
    await crowdfund.write.setCourseSaleParams(
      [courseNFT.address, coursePrice, creatorShare, backerShare],
  ...
  it('buyer receives CourseNFT and revenue is split correctly on purchase', async function () {
    const {
      ...
      backerShare,
      platformShare,
    } = await loadFixture(deployCourseSalesFixture)
    ...
    await token.write.approve([crowdfund.address, coursePrice], {
      account: buyer1.account,
    })
    const creatorBalBefore = await token.read.balanceOf([
      creator.account.address,
    ])
    const platformBalBefore = await token.read.balanceOf([
      platform.account.address,
    ])
    ...
    expect(getAddress(await courseNFT.read.ownerOf([1n]))).to.equal(
    ...
    const creatorBalAfter = await token.read.balanceOf([
      creator.account.address,
    ])
    const platformBalAfter = await token.read.balanceOf([
      platform.account.address,
    ])
  ...
  it('backers can withdraw proportional share after multiple purchases and balances update correctly', async function () {
    ...
    await token.write.approve([crowdfund.address, pledge1], {
      account: backer1.account,
    })
    await crowdfund.write.pledge([pledge1], { account: backer1.account })

    await token.write.approve([crowdfund.address, pledge2], {
      account: backer2.account,
    })
    ...
    const singleDust =
      coursePrice - singleCreatorAmt - singleBackerAmt - singlePlatformAmt
    ...
    const backer1Contrib = await crowdfund.read.contributions([
      backer1.account.address,
    ])
    const backer2Contrib = await crowdfund.read.contributions([
      backer2.account.address,
    ])
    ...
    await expect(
      crowdfund.write.withdrawBackerRevenue({ account: backer1.account }),
    ...
    const already1 = await crowdfund.read.backerWithdrawn([
      backer1.account.address,
    ])
    const already2 = await crowdfund.read.backerWithdrawn([
      backer2.account.address,
    ])
