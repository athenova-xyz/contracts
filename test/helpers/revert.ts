  ...
  try {
    ...
    for (const p of rpcErrorPatterns) {
      if (msg.toLowerCase().includes(p.toLowerCase())) {
        expect.fail(
          `RPC/network error encountered while expecting revert: ${msg}`,
        )
      }
    }
    ...
    const req =
      (err &&
        (err.request || err.requestArguments || err.args || err.transaction)) ||
      {}
    ...
    if (to && data) {
      try {
        ...
        const callMsg =
          callErr && callErr.message ? String(callErr.message) : String(callErr)
